...
window.addEventListener('load',()=>{
 ...
 const report=()=>{if(status.dataset.error)return;const s=window.combatState,p=s?.combatants?.player,e=window.__world3d,r=e?.zone?.interactables.find(r=>r.id==='tower_last_step');status.textContent=JSON.stringify({location:window.mapState?.currentLocation,active:s?.active,turn:s?.turnOrder?.[s.currentTurnIndex],ap:s?.apRemaining,mp:p?.mp,position:p?.position,rendered:e?.actor?.position,physicalContext:e?.physicalContext?.entityId,stairReach:r&&e.physicalReach(r),stairContext:e?.hasPhysicalInteraction('tower_last_step'),spell:s?.selectedSpell?.id,locked:e?.combatController?.actionLocked,lastPointer:panel.dataset.pointer},null,1);};
 const endingFixture=document.createElement('button');endingFixture.textContent='Open earned Sword ending fixture';panel.insertBefore(endingFixture,status);endingFixture.onclick=()=>{if(window.__world3d?.zone?.id!=='tower_antechamber'||window.combatState?.active)return;for(let i=0;i<3;i++)delete window.sceneState.flags['tower_epilogue_sword_'+i];setFlag('faced_the_shattered_god');setFlag('npc_dead_shattered_god');runScene('tower_ending_sword');};
