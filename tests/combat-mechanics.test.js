...
const browser={};vm.createContext(browser);for(const file of ['rules','tactical-combat','gameplay-catalog','combat-mechanics','action-pipeline'])vm.runInContext(fs.readFileSync(require.resolve('../site/'+file+'.js'),'utf8'),browser);
for(const [cls,spells] of Object.entries(Catalog.CLASS_SPELLS))for(const spell of spells){
 test(`${cls}/${spell.id}: identical seeded solo/server effects, one cost, rejection before mutation`,()=>{
  ...
  const cmd={id:'cast',encounterId:'battle',revision:0,actorId:'p',type:'spell',spellId:spell.id,targetId};
