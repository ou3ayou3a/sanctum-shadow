...
test('3D party positions are finite, bounded, and presentation-only',()=>{
  ...
  const normalized=Presence.normalizeWorldPosition({x:999,z:-999,rotation:Math.PI*5,state:'run',locationId:'vaelthar_city<script>'});
  assert.equal(normalized.x,100);assert.equal(normalized.z,-100);assert.equal(normalized.state,'run');assert.equal(normalized.locationId,'vaelthar_cityscript');assert.ok(Math.abs(normalized.rotation-Math.PI)<1e-12);
});
