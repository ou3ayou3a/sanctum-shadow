...
test('every physical quest target has a reachable approach using the actual zone collision catalog',()=>{
  for(const [id,target]of Object.entries(Flow.TARGETS)){
    ...
    const start=target.location==='temple_quarter'?{x:0,z:-4.85}:target.location==='mol_well_shaft'?{x:-2,z:-1.4}:['archive_level_four','archive_foundation','tower_antechamber'].includes(target.location)?{x:0,z:-6.35}:target.location==='church_archive'?{x:0,z:-3.35}:target.location==='archive_scriptorium'?{x:0,z:-2.85}:target.location==='harren_hall'?{x:0,z:-5.35}:target.location==='monastery_depths'?{x:0,z:-8.35}:target.location==='monastery_cellar'?{x:0,z:-6.35}:{x:0,z:17};
