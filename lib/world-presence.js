...
function normalizeWorldPosition(payload){
  ...
  return{x:clamp(x,-WORLD_LIMIT,WORLD_LIMIT),z:clamp(z,-WORLD_LIMIT,WORLD_LIMIT),rotation:normalizeRotation(payload.rotation),state,speed:clamp(finite(payload.speed)??0,0,6),locationId};
}
