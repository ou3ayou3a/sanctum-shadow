...
export class CityAtmosphere{
  ...
  update(dt,time){
    ...
    // The sky module owns texture backgrounds. Only legacy flat Color
    // backgrounds can be interpolated here; fog remains atmosphere-owned.
    if(this.engine.scene.background?.isColor)this.engine.scene.background.lerp(sky,1-Math.exp(-dt*.55));
    this.engine.scene.fog?.color.lerp(sky.clone().multiplyScalar(.72),1-Math.exp(-dt*.55));
