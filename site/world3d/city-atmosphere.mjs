...
const DAY=new THREE.Color(0x9bb7b1),DAWN=new THREE.Color(0xa86f58),NIGHT=new THREE.Color(0x101927);
...
export class CityAtmosphere{
  ...
  update(dt,time){
    ...
    if(this.engine.hemisphereLight){this.engine.hemisphereLight.intensity=.32+daylight*2.05;this.engine.hemisphereLight.color.copy(NIGHT).lerp(new THREE.Color(0xd9e1ca),daylight);}
    if(this.engine.sunLight){this.engine.sunLight.intensity=.22+daylight*3.0;const angle=(hour-6)/24*Math.PI*2;this.engine.sunLight.position.set(Math.cos(angle)*28,5+daylight*24,Math.sin(angle)*20);this.engine.sunLight.color.set(warm>.2?0xffbd79:0xffefd2);}
    this.engine.renderer.toneMappingExposure=.76+daylight*.38;
    const night=1-daylight;for(const light of this.torches)light.intensity=(light.userData.dayIntensity||5)*(.24+night*.92);
