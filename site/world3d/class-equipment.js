...
function attach(model,boneName,object,{position=[0,0,0],rotation=[0,0,0]}={}){const bone=model.getObjectByName(boneName);if(!bone)return null;object.position.set(...position);object.rotation.set(...rotation);object.traverse(o=>{if(o.isMesh)o.castShadow=true;});bone.add(object);return object;}
...
function stow(parent,object,position,rotation,scale=.012){object.scale.setScalar(scale);object.position.set(...position);object.rotation.set(...rotation);parent.add(object);return object;}

export function equipClass(actor,profile){
  const gear=new THREE.Group();gear.name=`class-gear:${profile.id}`;actor.add(gear);
  if(profile.weapon==='greatsword'){attach(actor.model,'mixamorig:RightHand',sword(profile),{position:[0,8,0],rotation:[0,0,.05]});stow(gear,sword(profile),[.42,.78,-.22],[.08,.12,-.5]);}
  if(profile.weapon==='mace_shield'){attach(actor.model,'mixamorig:RightHand',mace(profile),{position:[0,7,0]});attach(actor.model,'mixamorig:LeftForeArm',shield(profile),{position:[0,16,7],rotation:[0,0,0]});stow(gear,mace(profile),[.36,.72,-.22],[.05,.1,-.35]);stow(gear,shield(profile),[-.38,1.02,-.24],[0,0,.08]);}
  if(profile.weapon==='holy_staff'){attach(actor.model,'mixamorig:RightHand',staff(profile,false),{position:[0,4,0],rotation:[0,0,.08]});stow(gear,staff(profile,false),[.4,.46,-.25],[.04,.08,-.18]);}
  if(profile.weapon==='arcane_staff'){attach(actor.model,'mixamorig:RightHand',staff(profile,true),{position:[0,4,0],rotation:[0,0,.08]});stow(gear,staff(profile,true),[.4,.46,-.25],[.04,.08,-.18]);}
  if(profile.weapon==='twin_daggers'){attach(actor.model,'mixamorig:RightHand',dagger(profile),{position:[0,5,0]});attach(actor.model,'mixamorig:LeftHand',dagger(profile),{position:[0,5,0]});stow(gear,dagger(profile),[.32,.72,-.28],[.1,.1,-.7],.014);stow(gear,dagger(profile),[-.32,.72,-.28],[-.1,-.1,.7],.014);}
  if(profile.weapon==='bow_quiver'){
    attach(actor.model,'mixamorig:LeftHand',bow(profile),{position:[0,7,0],rotation:[0,.15,0]});stow(gear,bow(profile),[-.38,1.05,-.27],[.08,.15,-.35]);
  ...
  const aura=new THREE.Mesh(new THREE.RingGeometry(.58,.65,36),new THREE.MeshBasicMaterial({color:profile.accent,transparent:true,opacity:0,side:THREE.DoubleSide,depthWrite:false}));aura.rotation.x=-Math.PI/2;aura.position.y=.035;gear.add(aura);actor.classAura=aura;actor.classGear=gear;return gear;
}
