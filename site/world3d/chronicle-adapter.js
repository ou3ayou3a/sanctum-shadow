...
import {refreshPhysicalQuestTargets} from './physical-quest-targets.mjs?v=234';
...
export class Chronicle3DAdapter{
  ...
  targetFor(objective,quest){
    ...
    if(pendingId){const definition=window.PhysicalQuestFlow.TARGETS[pendingId],id=definition.location===this.engine.zone.id||!this.engine.zone.id?pendingId:this.engine.zone.id==='mol_well_shaft'?'well_rope_exit':['monastery_depths','archive_level_four'].includes(this.engine.zone.id)?'interior_exit':definition.location==='archive_level_four'&&this.engine.zone.id==='church_archive'?'entrance_archive_scriptorium':definition.entrance;const record=this.engine.zone.interactables.find(item=>item.id===id);if(record)return{position:record.position,interaction:record};}
