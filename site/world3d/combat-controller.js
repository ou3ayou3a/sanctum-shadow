...
export class Combat3DController{
  ...
  async start(){
    ...
    const state=this.state(),origin=this.engine.actor.position.clone(),local=localCombatantId(),localPosition=state.combatants?.[local]?.position||state.combatants?.player?.position||{x:0,z:0};this.battleOrigin=state.tactical?.origin?new THREE.Vector3(state.tactical.origin.x,0,state.tactical.origin.z):origin.clone().sub(new THREE.Vector3(localPosition.x||0,0,localPosition.z||0));let ally=0,enemy=0;this.makeCover();
    const tasks=Object.values(state.combatants||{}).map(async combatant=>{
      this.hp.set(combatant.id,combatant.hp);
      if(combatant.id===local||(!window.mp?.sessionCode&&combatant.id==='player')){this.engine.actor.position.copy(this.battleOrigin).add(new THREE.Vector3(combatant.position.x,0,combatant.position.z));this.engine.actor.setCombatStance(true);this.engine.actor.userData.combatantId=combatant.id;this.records.set(combatant.id,{combatant,actor:this.engine.actor,owned:false,label:this.makeLabel(combatant),audioPosition:this.engine.actor.position.clone(),stepDistance:0,stepCount:0});return;}
      ...
      const desired=combatant.position?this.battleOrigin.clone().add(new THREE.Vector3(combatant.position.x,0,combatant.position.z)):origin.clone().add(new THREE.Vector3(Math.sin(angle)*radius,0,Math.cos(angle)*radius));const position=desired;
  ...
  makeHud(){
    ...
    hud.addEventListener('click',event=>{const button=event.target.closest('button');if(!button||this.actionLocked)return;const action=button.dataset.action;if(action==='attack')this.attack();else if(action==='item')window.combatItem?.();else if(action==='end')window.endPlayerTurn?.();else if(action==='spell')this.chooseSpell(button.dataset.spell);});
  ...
  chooseSpell(id){if(this.actionLocked||!this.isLocalTurn())return;window.selectSpell?.(id);this.sync(true);const spell=this.state()?.selectedSpell;if(spell?.id==='shadow_step'){this.engine.toast('Shadow Step: click a visible, unoccupied destination within 60 feet.');return;}if(window.CombatMechanics?.policies[spell?.id]?.[0]==='self'){this.cast(localCombatantId(),spell);return;}this.engine.toast('Spell readied — click a target to cast.');}
  handleKey(code){
    if(!this.active)return false;
    if(this.actionLocked)return true;
  ...
  attackAssessment(){const state=this.state(),local=state?.combatants?.[localCombatantId()]||state?.combatants?.player,target=state?.combatants?.[state?.selectedTarget];return window.TacticalCombat?.validateAttack(local,target,state?.tactical||{})||{ok:true,range:null,distance:null,coverBonus:0};}
  ...
  beginSoloAction({action='attack',spell=null,targetId,resolve}){
    ...
    const encounterId=this.state()?.encounterId,before=Number(this.state()?.combatants?.[targetId]?.hp),revision=this.state()?.commandRevision;
    // Rules and costs commit now. Animation callbacks never execute commands.
    resolve?.();if(this.state()?.commandRevision===revision)return false;
    const after=Number(this.state()?.combatants?.[targetId]?.hp);
    ...
    this.schedule(()=>{
      ...
      this.engine.abilityEffects?.present(profile,source,targetRecord.actor,{onImpact:()=>{
        if(this.state()?.encounterId!==encounterId||!this.active)return;
        this.impactActorId=localCombatantId();
  ...
  select(id){const state=this.state(),target=state?.combatants?.[id],spell=state?.selectedSpell;if(!target||(target.hp<=0&&!(spell?.id==='revivify'&&target.isPlayer)))return;const allySpell=target.isPlayer&&['heal','revive','buff'].includes(spell?.type);if(target.isPlayer&&!allySpell)return;window.selectTarget?.(id);if(spell&&this.isLocalTurn())this.cast(id,spell);this.sync(true);}
  ...
  presentAuthoritativeUpdate(nextState,presentation,apply){
    ...
    // Receive authoritative state immediately; effects only decorate the result.
    apply();const encounterId=nextState?.encounterId;
    ...
    this.schedule(()=>this.engine.abilityEffects?.present({...presentation,travelDuration},sourceRecord.actor,targetRecord.actor,{onImpact:()=>{
      if(this.state()?.encounterId!==encounterId||!this.active)return;
      this.impactActorId=presentation.actorId;if(!presentation.hit&&!presentation.healing)this.missReaction(targetRecord,presentation.id);
  ...
  move(point,run=false){
    ...
    if(state.selectedSpell?.id==='shadow_step'){window.castSelectedSpell?.({x:point.x-this.battleOrigin.x,z:point.z-this.battleOrigin.z});this.sync(true);return;}
    ...
    const tacticalPosition={x:Number((target.x-this.battleOrigin.x).toFixed(3)),z:Number((target.z-this.battleOrigin.z).toFixed(3))},local=state.combatants?.[localCombatantId()]||state.combatants?.player,validation=window.TacticalCombat?.validateMove(local?.position,tacticalPosition,{...state.tactical,maxDistance:state.tactical?.moveRange||MOVE_RANGE,occupied:Object.values(state.combatants).filter(c=>c.id!==local.id&&c.hp>0&&c.position).map(c=>({...c.position,radius:.45}))});if(validation&&!validation.ok){this.engine.toast(`Cannot move: ${validation.reason.replaceAll('_',' ')}.`);return;}
    const path=(validation?.path||[]).map(p=>({x:p.x+this.battleOrigin.x,y:0,z:p.z+this.battleOrigin.z}));if(!path.length){this.engine.toast('That route is blocked.');return;}
    // No optimistic walking in multiplayer: wait for the server position update.
    const revision=state.commandRevision;window.combatMove?.(validation.position);
    if(window.mp?.sessionCode){this.lockAction(Math.max(.5,validation.distance/2));return;}
    if(this.state()?.commandRevision===revision)return;
    this.engine.marker.position.copy(target);this.engine.marker.visible=true;this.lockAction(Math.max(.3,validation.distance/2));this.engine.actor.moveAlong(path,{run,onArrive:()=>{this.engine.marker.visible=false;this.sync(true);}});
  }

  sync(force=false){
    const state=this.state();if(!state?.active)return;const current=this.currentId();
    for(const [id,record]of this.records){const combatant=state.combatants?.[id];if(!combatant)continue;record.combatant=combatant;if(combatant.position){const key=`${combatant.position.x}:${combatant.position.z}`;if(record.positionKey&&record.positionKey!==key){if(combatant.teleportRevision&&record.teleportRevision!==combatant.teleportRevision){record.actor.stop();record.actor.position.set(combatant.position.x+this.battleOrigin.x,0,combatant.position.z+this.battleOrigin.z);record.teleportRevision=combatant.teleportRevision;}else{const route=(combatant.movementPath||[combatant.position]).map(p=>({x:p.x+this.battleOrigin.x,y:0,z:p.z+this.battleOrigin.z}));record.actor.moveAlong(route,{run:false});}}record.positionKey=key;}const previous=this.hp.get(id);if(previous<=0&&combatant.hp>0){record.deathUntil=0;record.actor.oneShotLocked=false;record.actor.setState('combat_idle',true);}if(previous!==undefined&&combatant.hp<previous){if(combatant.hp<=0)window.AudioEngine?.playWorldSound?.('death',record.actor.position,{emitterId:`combat:${id}`,eventId:`defeat:${state.round||0}:${id}:${previous}`});this.hitFeedback(record,previous-combatant.hp,combatant.hp<=0);}this.hp.set(id,combatant.hp);record.actor.visible=combatant.hp>0||(state.selectedSpell?.id==='revivify'&&combatant.isPlayer)||performance.now()<(record.deathUntil||0);record.actor.selectionRing?.material.color.set(id===state.selectedTarget?0xff5252:(combatant.isPlayer?0x71c99b:0xb7473f));record.actor.selectionRing?.scale.setScalar(id===current?1.35:1);record.label.disabled=combatant.hp<=0&&!(state.selectedSpell?.id==='revivify'&&combatant.isPlayer);}
