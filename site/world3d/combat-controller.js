...
export class Combat3DController{
  ...
  attack(){if(!this.isLocalTurn()){this.engine.toast('Wait for your turn.');return;}const state=this.state(),targetId=state?.selectedTarget;if(!targetId){this.engine.toast('Select an enemy first.');return;}const target=this.records.get(targetId)?.actor;this.engine.actor.playPrimaryAction();this.engine.abilityEffects?.basicAttack(this.engine.actor,target,this.engine.actor.characterClass);window.combatAttack?.();}
  ...
  hitFeedback(record,damage){record.actor.scale.multiplyScalar(.92);setTimeout(()=>record.actor?.scale.multiplyScalar(1/.92),130);const text=document.createElement('div');text.className='world3d-damage';text.textContent=`−${damage}`;this.engine.overlay.appendChild(text);this.positionElement(text,record.actor.position,2.4);setTimeout(()=>text.remove(),900);}
