...
export class Combat3DController{
  ...
  chooseSpell(id){if(!this.isLocalTurn())return;window.selectSpell?.(id);this.sync(true);const spell=this.state()?.selectedSpell;if(window.CombatMechanics?.policies[spell?.id]?.[0]==='self'){this.cast(localCombatantId(),spell);return;}this.engine.toast('Spell readied — click a target to cast.');}
  handleKey(code){
