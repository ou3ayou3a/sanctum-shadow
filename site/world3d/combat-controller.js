...
export class Combat3DController{
  ...
  async start(){
    ...
    const tasks=Object.values(state.combatants||{}).map(async combatant=>{
      ...
      if(combatant.id===local||(!window.mp?.sessionCode&&combatant.id==='player')){this.engine.actor.setCombatStance(true);this.records.set(combatant.id,{combatant,actor:this.engine.actor,owned:false,label:this.makeLabel(combatant)});return;}
  ...
  beginSoloAction({action='attack',spell=null,targetId,resolve}){
    ...
    this.schedule(()=>{
      ...
      this.engine.abilityEffects?.present(profile,source,targetRecord.actor,{onImpact:()=>{
        ...
        if(this.state()?.active===false&&Number.isFinite(before)&&after<before){targetRecord.combatant=this.state()?.combatants?.[targetId]||targetRecord.combatant;this.hp.set(targetId,after);this.hitFeedback(targetRecord,before-after,after<=0);this.endingUntil=performance.now()+(after<=0?1600:800);}else this.sync(true);this.impactActorId=null;
      }});
  ...
  select(id){const state=this.state(),target=state?.combatants?.[id];if(!target||target.hp<=0||target.isPlayer)return;window.selectTarget?.(id);if(state.selectedSpell&&this.isLocalTurn())this.cast(id,state.selectedSpell);this.sync(true);}
  ...
  presentAuthoritativeUpdate(nextState,presentation,apply){
    ...
    this.schedule(()=>this.engine.abilityEffects?.present({...presentation,travelDuration},sourceRecord.actor,targetRecord.actor,{onImpact:()=>{
      ...
      if(nextState?.active===false&&presentation.damage>0){const hp=Number(nextState.combatants?.[presentation.targetId]?.hp)||0;targetRecord.combatant=nextState.combatants?.[presentation.targetId]||targetRecord.combatant;this.hp.set(presentation.targetId,hp);this.hitFeedback(targetRecord,presentation.damage,hp<=0);this.endingUntil=performance.now()+(hp<=0?1600:800);}else this.sync(true);this.impactActorId=null;
    }}),releaseDelay*1000);
    return true;
  }
  ...
  renderHud(){
    ...
    const assessment=target?this.attackAssessment():null;this.hud.querySelector('.w3c-target').textContent=target?`Target: ${displayName(target)} · ${target.boss?'???':`${target.hp}/${target.maxHp}`} HP${assessment?.distance!==null?` · ${assessment.distance.toFixed(1)}m / ${assessment.range}m${assessment.coverBonus?' · COVER +2 AC':''}`:''}`:'Select an enemy';
    for(const button of this.hud.querySelectorAll('.w3c-actions [data-action]'))button.disabled=!this.isLocalTurn()||(button.dataset.action==='attack'&&assessment&&!assessment.ok);
