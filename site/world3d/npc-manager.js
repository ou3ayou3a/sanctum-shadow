...
export class NPCManager{
  ...
  async spawn(config,index){
    const actor=new CharacterActor({modelUrl:this.modelUrl,race:config.race||'human',characterClass:config.classId||'warrior',identity:config.appearanceId||config.id,role:config.role,title:config.title,action:config.action,appearance:config.appearance,scale:.94});actor.position.set(...config.position);actor.rotation.y=config.facing||0;actor.userData.npcId=config.id;this.engine.scene.add(actor);await actor.load();actor.mixer?.setTime((index*.731)%2.8);
    ...
    const label=document.createElement('button');label.type='button';label.className='world3d-npc-label';label.dataset.npcAction=config.action||'dialogue';label.setAttribute('aria-label',`${config.action==='shop'?'Trade with':'Speak with'} ${config.name}`);label.innerHTML=`<strong>${config.name}</strong><span>${config.title}</span>`;this.engine.overlay.appendChild(label);
    ...
    const record={config,actor,collider,label,interaction,patrolIndex:0,nextPatrol:4+index*.7,lastSchedule:'',ambientCycle:index,nextAmbient:3+index*.43};label.addEventListener('click',()=>{actor.stop();this.engine.goToInteraction(interaction);});this.records.push(record);this.engine.registerInteraction(interaction);this.applySchedule(record,true);
  }
  interact(record){record.actor.stop();record.actor.turnToward(this.engine.actor);record.actor.playOneShot('interact');const config=record.config;if(config.action==='shop'){window.openShop?.();return;}if(config.action==='ambient'){this.engine.toast(config.ambientLine||`${config.name} nods, then returns to work.`,4200);return;}window.startNPCConversation?.(config.dialogueId||config.id);}
  ...
  update(dt,time){
    for(const record of this.records){this.applySchedule(record);const {actor,collider,config}=record;if(!actor.visible)continue;const distanceToCamera=actor.position.distanceTo(this.engine.camera.position);if(distanceToCamera<28||actor.path.length)actor.update(dt);collider.position.copy(actor.position);collider.position.y=1.45;
      ...
      const world=new THREE.Vector3(actor.position.x,2.3*actor.profile.proportions[1],actor.position.z),projected=world.clone().project(this.engine.camera);const rect=this.engine.canvas.getBoundingClientRect(),distanceToPlayer=actor.position.distanceTo(this.engine.actor.position);const visible=projected.z<1&&projected.x>-1.1&&projected.x<1.1&&projected.y>-1.1&&projected.y<1.1&&distanceToCamera<24&&distanceToPlayer<13;
